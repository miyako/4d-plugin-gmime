...
JSONNODE *json_parse(CUTF16String &t)
{
    JSONNODE *n = NULL;
#if VERSIONWIN
    n= json_parse((json_const json_char *)t.c_str());
#else
    CFStringRef str = CFStringCreateWithCharacters(kCFAllocatorDefault, (const UniChar *)t.c_str(), t.length());
    if(str)
    {
        size_t size = CFStringGetMaximumSizeForEncoding(CFStringGetLength(str), kCFStringEncodingUTF32LE) + sizeof(wchar_t);
        std::vector<uint8_t> buf(size);
        CFIndex len = 0;
        CFIndex count = CFStringGetBytes(str, CFRangeMake(0, CFStringGetLength(str)), kCFStringEncodingUTF32LE, 0, true, (UInt8 *)&buf[0], size, &len);
        std::wstring u32 = std::wstring((wchar_t *)&buf[0], count);
        CFRelease(str);
        n = json_parse((json_const json_char *)u32.c_str());
    }
#endif
    return n;
}
